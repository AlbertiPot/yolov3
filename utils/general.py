...
def check_dataset(dict):
    ...
    if val and len(val):
        val = [Path(x).resolve() for x in (val if isinstance(val, list) else [val])]  # val path resolve()将相对路径变为绝对路径
...
def strip_optimizer(f='weights/best.pt', s=''):  # from utils.general import *; strip_optimizer()
    ...
    torch.save(x, s or f) # 存入s或覆盖f，s在前，s若为空，则存入f
...
def increment_path(path, exist_ok=True, sep=''):
    ...
    if (path.exists() and exist_ok) or (not path.exists()): # 检查路径存在：若当前路径存在且允许覆盖则返回当前路径，若不存在直接返回
        return str(path)
    else:
        dirs = glob.glob(f"{path}{sep}*")  # similar paths  # glob查找匹配的路径，*是通配符
        matches = [re.search(rf"%s{sep}(\d+)" % path.stem, d) for d in dirs]    #stem文件不带后缀名
